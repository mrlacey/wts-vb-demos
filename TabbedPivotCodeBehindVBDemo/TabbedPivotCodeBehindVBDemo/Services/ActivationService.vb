Imports TabbedPivotCodeBehindVBDemo.Activation
...
Namespace Services
    ...
    Friend Class ActivationService
        ...
        Private Async Function InitializeAsync() As Task
            Await Singleton(Of StoreNotifications5Service).Instance.InitializeAsync()
            Await Singleton(Of LiveTile5Service).Instance.EnableQueueAsync()
        ...
        Private Async Function StartupAsync() As Task
            ...
            Singleton(Of LiveTile5Service).Instance.SampleUpdate()

            ' TODO WTS: To use the HubNotificationService specific data related with your Azure Notification Hubs is required.
            '  1. Go to the HubNotifications5Service class, in the InitializeAsync() method, provide the Hub Name and DefaultListenSharedAccessSignature.
            '  2. Uncomment the following line (an exception will be thrown if it is executed and the above information is not provided).
            ' Await Singleton(Of HubNotifications5Service).Instance.InitializeAsync()
        ...
        Private Iterator Function GetActivationHandlers() As IEnumerable(Of ActivationHandler)
            ...
            yield Singleton(Of ToastNotifications5Service).Instance
            Yield Singleton(Of SuspendAndResumeService).Instance
            yield Singleton(Of StoreNotifications5Service).Instance
            yield Singleton(Of ShareTarget5ActivationHandler).Instance
            yield Singleton(Of LiveTile5Service).Instance
            yield Singleton(Of HubNotifications5Service).Instance
