Imports TabbedPivotMVVMBasicVBDemo.Activation
...
Namespace Services
    ...
    Friend Class ActivationService
        ...
        Private Async Function InitializeAsync() As Task
            Await Singleton(Of StoreNotifications3Service).Instance.InitializeAsync()
            Await Singleton(Of LiveTile3Service).Instance.EnableQueueAsync()
        ...
        Private Async Function StartupAsync() As Task
            ...
            Singleton(Of LiveTile3Service).Instance.SampleUpdate()

            ' TODO WTS: To use the HubNotificationService specific data related with your Azure Notification Hubs is required.
            '  1. Go to the HubNotifications3Service class, in the InitializeAsync() method, provide the Hub Name and DefaultListenSharedAccessSignature.
            '  2. Uncomment the following line (an exception will be thrown if it is executed and the above information is not provided).
            ' Await Singleton(Of HubNotifications3Service).Instance.InitializeAsync()
        ...
        Private Iterator Function GetActivationHandlers() As IEnumerable(Of ActivationHandler)
            ...
            yield Singleton(Of ToastNotifications3Service).Instance
            Yield Singleton(Of SuspendAndResumeService).Instance
            yield Singleton(Of StoreNotifications3Service).Instance
            yield Singleton(Of ShareTarget3ActivationHandler).Instance
            yield Singleton(Of LiveTile3Service).Instance
            yield Singleton(Of HubNotifications3Service).Instance
